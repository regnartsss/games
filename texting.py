...
def text_mining(user_mining):
    text = ""
    if user_mining["resource"] == "iron":
        text = " 👣 Ты спустился в шахту и увидел %s кусок \n⛓ Металла.\n"% (t_m[user_mining["resource"]][user_mining["lvl"]])
    elif user_mining["resource"] == "wood":
        text = " 👣 Ты подошел к %s стволу \n🌲 Дерева.\n" % (t_m[user_mining["resource"]][user_mining["lvl"]])
    elif user_mining["resource"] == "stone":
        text = " 👣 Ты вошёл в пещеру и наткнулся на %s \n🧱 Булыжник.\n" % (t_m[user_mining["resource"]][user_mining["lvl"]])
    text += "🏵 Уровень: %s\n" \
            "❗ Количество ресурса: %s\n" \
            "🪓  Добыча: 2 штуки за 1 секунду.\n" \
            "⏱ Сбор ресурса:  %s" % \
            (user_mining["lvl"], user_mining["number"],
             user_mining["timer"])
    return text

text_training_uplvl = "Увеличьте уровень %s"
